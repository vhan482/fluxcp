...
<div class="col-12 col-md-8">
	...
	<form action="<?php echo $this->urlWithQs ?>" method="post">
		...
		<div class="form-group">
			...
			<small class="form-text text-muted"><?php echo htmlspecialchars(Flux::message('ResendServerInfo')) ?></small>
		</div>
		<?php endif ?>
		<div class="form-group">
			<label for="userid"><?php echo htmlspecialchars(Flux::message('ResendAccountLabel')) ?></label>
			<input class="form-control" type="text" name="userid" id="userid" />
			<small class="form-text text-muted"><?php echo htmlspecialchars(Flux::message('ResendAccountInfo')) ?></small>
		</div>
		<div class="form-group">
			<label for="email"><?php echo htmlspecialchars(Flux::message('ResendEmailLabel')) ?></label>
			<input class="form-control" type="text" name="email" id="email" />
			<small class="form-text text-muted"><?php echo htmlspecialchars(Flux::message('ResendEmailInfo')) ?></small>
		</div>
		<input class="btn btn-primary btn-block" type="submit" value="<?php echo htmlspecialchars(Flux::message('ResendButton')) ?>" />
	</form>
</div>
</div>